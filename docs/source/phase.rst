...
and can be extracted using :func:`pyholoscope.amplitude` and :func:`pyholoscope.phase`.
...
``set_relative_phase(True)`` (or by setting the ``background`` and ``relativePhase``
...
be returned. The function assumes that the inputs as phase maps if the datatype is not complex.
...
For example, assuming we have a complex field in ``hologram_demod``::
    ...
    unwrappedPhase = pyholoscope.phase_unwrap(hologram_demod)
    tilt = pyholoscope.obtain_tilt(unwrappedPhase)
    correctedPhase = pyholoscope.relative_phase(unwrappedPhase, tilt)
    
This returns the unwrapped and tilt-corrected phase map in ``correctedPhase``.   
...
along with the complex field or phase map, for example::
    ...
    correctedPhase = pyholoscope.relative_phase_self(hologram_demod, roi)
...
and a pixel values of 65535 corresonds to 2pi. The function accepts either phase maps or complex fields.
If a phase map is provided, any phase values outside of the [0, 2pi] interval will be wrapped to this 
interval. 

To save unwrapped phase maps, save the unwrapped phae map (a float array) using 
...
responsibility to ensure the range is sensible. In both cases, the absolute phase values cannot be obtained from
the image unless a separate record is kept
