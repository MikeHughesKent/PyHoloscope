...
for gridSize in gridSizes:
    testcode = (
        "pyh.propagator(gridSize, wavelength, pixelSize, depth, precision = 'single')"
    )
...
for gridSize in gridSizes:
    prop = pyh.propagator_numba((gridSize, gridSize), wavelength, pixelSize, depth)
    img = np.random.random((gridSize, gridSize)) + 1j * np.random.random(
        (gridSize, gridSize)
    )
    testcode = "pyh.refocus(img, prop)"
    t = timeit.timeit(stmt=testcode, number=10, globals=globals())
...
for gridSize in gridSizes:
    holo = pyh.Holo(
        mode=pyh.INLINE, wavelength=wavelength, pixelSize=pixelSize, depth=depth
    )
    img = np.random.random((gridSize, gridSize))
