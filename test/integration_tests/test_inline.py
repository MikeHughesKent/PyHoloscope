...
holo.update_auto_window(
    hologram
)  # To make timing below just for propagator/window generation
...
recon = holo.process(hologram)
print(
    f"Inline refocusing time with window: {round((time.perf_counter() - t1) * 1000)} ms"
)
plt.figure(dpi=150)
plt.imshow(pyh.amplitude(recon), cmap="gray")
plt.title("Refocused Hologram, pre-window")
...
recon = holo.process(hologram)
print(
    f"Inline refocusing time with post window: {round((time.perf_counter() - t1) * 1000)} ms"
)
plt.figure(dpi=150)
plt.imshow(pyh.amplitude(recon), cmap="gray")
plt.title("Refocused Hologram, pre and post window")
...
plt.figure(dpi=150)
plt.imshow(pyh.amplitude(recon), cmap="gray")
plt.title("Refocused Hologram, no window")
...
recon = holo.process(hologram)
print(
    f"Inline refocusing time with no background or normalisation: {round((time.perf_counter() - t1) * 1000)} ms"
)
plt.figure(dpi=150)
plt.imshow(pyh.amplitude(recon), cmap="gray")
plt.title("Refocused Hologram, no background or norm")
