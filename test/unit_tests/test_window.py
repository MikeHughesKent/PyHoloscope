...
class TestWindow(unittest.TestCase):
    ...
    def test_create(self):
        ...
        window_low_level = pyh.square_cosine_window(
            self.hologram,
            (np.shape(self.hologram)[1] / 2, np.shape(self.hologram)[0] / 2),
            10,
        )
        assert np.array_equal(window_low_level, holo.window) == True
    ...
    def test_autowindow(self):
        ...
        window_low_level = pyh.square_cosine_window(self.hologram, 100, 10)
        assert np.array_equal(window_low_level, holo.window) == True
        ...
        window_low_level = pyh.square_cosine_window(self.hologram, 100, 20)
        assert np.array_equal(window_low_level, holo.window) == True
    ...
    def test_resize(self):
        ...
        window_low_level = pyh.square_cosine_window(
            int(np.shape(self.hologram)[0] / 2), 100, 20
        )
        holo.set_window(window_low_level)
