...
# Set the auto focus parameters
holo.set_find_focus_parameters(depth_range = depth_range, roi = roi, method = 'sum')

# Autofocus the hologram
refocused = holo.auto_focus(hologram)


print(f"Focus found at: {round(1000 * holo.depth,3)} mm")
...
plt.figure(dpi=300);plt.title(f"Autofocused"); plt.imshow(pyh.amp(refocused), cmap="gray")
